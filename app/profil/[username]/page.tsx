...
export default async function ProfilePage({ params }: Props) {
  ...
  const [comments, likedArticles, followers, following, isFollowing] =
    await Promise.all([
  ...
  const commentCount = await prisma.comment.count({ where: { userId: user.id } });
    ...
    <div className="flex flex-col gap-5 px-3 py-4 sm:px-0 sm:pt-6">
      ...
      <section className="flex flex-col gap-4 rounded-lg border border-neutral-200 bg-white p-5 sm:flex-row sm:items-start sm:gap-6">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-2">
          ...
          <div className="mt-1 flex divide-x divide-neutral-200 self-start rounded-md border border-neutral-200 py-2">
            <span className={STAT}>
              <b className="text-lg font-black text-neutral-900">{commentCount}</b>
